...
//  BrainActivitySwift
//
//  Created by Victor Gelmutdinov on 25/08/16.
...
public class AnimationView: UIView {
    ...
    var visibleAnimation = false {
        didSet {
            for sl in self.segmentLayers {
                sl.hidden = !visibleAnimation
            }
        }
    }
    private var disassembly = true
    ...
    func disassemblyAnimation(){
        if self.assembledAmount >= 2 {
            disassembly = true
        }else if assembledAmount <= 0{
            disassembly = false
        }
        if disassembly {
            assembledAmount -= 0.05
        }else{
            assembledAmount += 0.05
        }
    }
